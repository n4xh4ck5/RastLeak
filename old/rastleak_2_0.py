...
if option == 1:
	...
	#Into the target
	try:
		catpcha=SearchGoogle(N,target,option)
	except Exception as e:
		catpcha = False
		print e
		pass
	if catpcha == True:
		print "Do yo like to use Bing to go on the leak information into target (Y/N)"
		resp = raw_input().lower()
		if (resp =='y'):
...
#option ==2
	...
	SearchGoogle(N,target,option)
#Called the function to display the results
ShowResults(newlist,num_files,target,output,option,catpcha)